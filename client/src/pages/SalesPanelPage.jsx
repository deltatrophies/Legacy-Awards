...
export default function SalesPanelPage() {
  ...
  useEffect(() => { document.title = `${section === "orders" ? "Paid Orders" : section === "leads" ? "Sales Leads" : "Sales Dashboard"} - Award Arts`; }, [section]);
  useEffect(() => {
    let checking = false;
    ...
    <SalesGuard>
      <main className="admin-shell sales-shell">
        <aside className="admin-sidebar sales-sidebar">
          <Link className="admin-brand" to="/sales/dashboard">Award Arts Sales</Link>
...
function SalesInquiryDetail({ inquiry, user, busy, onBack, onPerform }) {
  ...
  const whatsapp = number ? `https://wa.me/${number}?text=${encodeURIComponent(`Hi ${inquiry.name || "there"}, this is Award Arts regarding enquiry ${inquiry.reference}.`)}` : "";
...
function SalesQuoteDetail({ quote, team, user, busy, onBack, onPerform }) {
  ...
  const whatsapp = number ? `https://wa.me/${number}?text=${encodeURIComponent(`Hi ${quote.customer?.name || "there"}, this is Award Arts regarding quotation ${quote.reference}.`)}` : "";
...
function SalesOrderDetail({ order, team, user, busy, onBack, onPerform }) {
  ...
  const whatsapp = number ? `https://wa.me/${number}?text=${encodeURIComponent(`Hi ${order.customer?.name || "there"}, this is Award Arts regarding paid order ${order.reference}.`)}` : "";
