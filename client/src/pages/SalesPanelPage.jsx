      ...
      <main className="admin-shell sales-shell">
        ...
        <section className="admin-main">
          ...
          <div className="sales-view-filter" aria-label="Work ownership filter">
            {["all", "mine", "unassigned"].map((item) => <button type="button" className={view === item ? "active" : ""} key={item} onClick={() => setView(item)}>{item === "all" ? (canManage ? "All team work" : "My work + open queue") : item === "mine" ? "Assigned to me" : "Unassigned queue"}</button>)}
          </div>
          {loading ? <p className="admin-empty">Loading your pipeline...</p> : null}
          {!loading && section === "dashboard" ? <SalesDashboard summary={summary} quotes={quotes} navigate={navigate} /> : null}
...
function SalesDashboard({ summary, quotes, navigate }) {
  const cards = [["Assigned leads", summary.assignedLeads || 0], ["Open queue", summary.unassignedLeads || 0], ["Needs attention", summary.needsAttention || 0], ["Follow-ups due", summary.followUpsDue || 0], ["Open paid orders", summary.openOrders || 0], ["Total paid orders", summary.paidOrders || 0]];
  ...
  return <div className="admin-dashboard"><section className="admin-metric-grid">{cards.map(([label, value]) => <article className="admin-metric" key={label}><span>{label}</span><strong>{value}</strong></article>)}</section><section className="admin-panel"><div className="admin-section-heading"><div><p className="admin-eyebrow">Priority desk</p><h2>Customer actions requiring attention</h2></div></div>{attention.length ? <div className="admin-order-stack">{attention.map((quote) => <button className="sales-attention-row" key={quote.reference} type="button" onClick={() => navigate(`/sales/leads/${encodeURIComponent(getId(quote))}`)}><span><strong>{quote.reference}</strong><small>{quote.customer?.name}</small></span><b>{quote.customerDecision === "accepted" ? "Quote accepted" : "Contact requested"}</b></button>)}</div> : <p className="admin-empty">No urgent customer action right now.</p>}</section></div>;
}
...
function OwnershipControl({ record, team, user, busy, kind, onPerform }) {
  ...
  if (!record.assignedTo) return <button className="admin-primary-button" disabled={Boolean(busy)} type="button" onClick={() => onPerform(`claim-${getId(record)}`, () => kind === "quote" ? salesApi.claimQuote(getId(record)) : salesApi.claimOrder(getId(record)), `${kind === "quote" ? "Lead" : "Order"} assigned to you.`)}>Claim {kind === "quote" ? "lead" : "order"}</button>;
